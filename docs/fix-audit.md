...
| PENDING todo 36 fix SHA | **RED:** `tests/repro/run-h9.sh` on R1 base `f04a90e95c5018b693f267a9dcdd445c20ec6e3a` captures 89 bytes of failed-open diagnostics and 28 bytes of version banner on stdout. **GREEN:** the same reproducer captures **0 bytes** on stdout in both paths; `tests/unit/unit_logging.cpp` verifies empty stdout plus original error/banner substrings and `librga:` context on stderr in no-device and fake-device runs. | host-shim-only | `abidiff` of matched arm64 Trixie R1-base and branch `librga.so.2.1.0`: clean exit, no removed or changed exported symbol | PENDING independent review receipt before todo 46 may merge this fix | Not reported upstream; review determines a source contribution path |
...
cleanly with no removed or changed exported symbol.
