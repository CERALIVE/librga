...
if [[ ${1:-} == --selftest ]]; then
    scratch=$(mktemp -d "${MESON_BUILD_ROOT:-.}/conversion-evidence.XXXXXX")
    ...
    for fixture in legacy im2d; do
        rga_conversion_logged "$scratch/$fixture" || exit 1
    done
    ...
    printf 'PASS: both success markers accepted; enabled-only, failed and empty rejected\n'
else
    [[ $# == 1 ]] || exit 2
