// Modified by CeraLive 2026-09-16: recover R0 modes and warm R1's two sessions.
#define _GNU_SOURCE
#include <dirent.h>
#include <dlfcn.h>
#include <errno.h>
...
static int run_cell(const struct cell *c, int iterations, int legacy, enum oracle_filter filter, double minimum)
{
    ...
    *(void **)(&end)=dlsym(RTLD_DEFAULT,"rga_timing_end");
    ...
    if (!completed || (deadline && now_us()<deadline)) goto done;
    result=0;
done:
    release(&src); release(&dst); free(expected); free(host); return result;
}
static int warm_im2d(void)
{
    rga_buffer_t probe=wrapbuffer_fd(100,64,64,RK_FORMAT_YCbCr_420_SP,64,64);
    im_rect zero={0};
    return imcheck(probe,probe,zero,zero)!=IM_STATUS_NOERROR;
}
int main(int argc, char **argv)
{
    int iterations=10,selftest=0,explicit_csc=0,core=0,routing=0,soak=0,imonly=0;
    int session_selftest=0; double minimum=30;
    enum oracle_filter filter=ORACLE_BOX;
    for (int i=1;i<argc;i++) {
        if (!strcmp(argv[i],"--selftest")) { selftest=1; iterations=1; minimum=INFINITY; }
        else if (!strcmp(argv[i],"--session-selftest")) session_selftest=1;
        ...
        else { fprintf(stderr,"usage: %s [--selftest|--session-selftest|--routing|--soak] [--core 1|2|4] [--improcess-only] [--iterations N] [--explicit-csc] [--bilinear]\n",argv[0]); return 2; }
    }
    if (selftest+session_selftest+routing+soak>1) return 2;
    if (session_selftest && !dlsym(RTLD_DEFAULT,"fake_rga_active")) return 77;
    if (!session_selftest && dlsym(RTLD_DEFAULT,"fake_rga_active")) { fprintf(stderr,"refusing fake RGA for hardware bench\n"); return 1; }
    if (!session_selftest && (access("/dev/rga",R_OK|W_OK) || access("/dev/dma_heap/system",R_OK|W_OK))) { perror("hardware preflight"); return 77; }
    if (c_RkRgaInit()) return 1;
    void *context=NULL;
    c_RkRgaGetContext(&context);
    if (!context || warm_im2d()) { fprintf(stderr,"RGA session initialization failed\n"); return 1; }
    ...
    if (session_selftest) { c_RkRgaGetContext(&context); rc=context==NULL || warm_im2d(); }
        ...
        const struct cell c={"soak-4k-nv16",3840,2160,3840,2160,RK_FORMAT_YCbCr_422_SP,0,0,0};
        double start=now_us(); deadline=start+3600e6;
        rc=run_cell(&c,iterations,0,filter,minimum);
        printf("soak_elapsed_seconds=%.6f\n",(now_us()-start)/1e6);
    }
